class AvatarHandler {
    ...
    compressImage() {
        ...
        console.log('Сжатое изображение:', width, 'x', height);

        this.showCropModal(); // Шаг 3
    }
    ...
    showCropModal() {
        ...
        this.cropImage.onload = () => {
            console.log('Изображение загружено в модальном окне');
            this.setupCropArea();
    ...
    setupCropArea() {
        ...
        console.log('Размеры изображения в модальном окне:', imgWidth, 'x', imgHeight);
    ...
    updateCropArea() {
        ...
        console.log('Область обрезки обновлена:', {
            left: this.cropArea.style.left,
            top: this.cropArea.style.top,
            width: this.cropArea.style.width,
            height: this.cropArea.style.height
        });
    }
    ...
    // Шаги 4-6: Обрезка, сжатие до 100x100 и загрузка
    cropAndUpload() {
        console.log('Начинаем обрезку и загрузку');
        
        // Шаг 4: Обрезка с правильным расчетом масштаба
        const canvas = document.createElement('canvas');
        const ctx = canvas.getContext('2d');
        canvas.width = 100;
        canvas.height = 100;
        ...
        sourceImage.onload = () => {
            ...
            console.log('Отображаемое изображение:', this.cropImage.offsetWidth, 'x', this.cropImage.offsetHeight);
            
            // Рассчитываем масштаб между исходным и отображаемым изображением
            const scaleX = sourceImage.width / this.cropImage.offsetWidth;
            const scaleY = sourceImage.height / this.cropImage.offsetHeight;
            ...
            this.uploadAvatar(finalDataUrl); // Шаг 6
        };
        sourceImage.src = this.compressedImage;
    }
    ...
    uploadAvatar(dataUrl) {
        ...
        .then(data => {
            if (data.success) {
                ...
                this.closeCropModal();
                this.updateAvatarDisplay(data.avatar_path); // Шаг 7
            } else {
